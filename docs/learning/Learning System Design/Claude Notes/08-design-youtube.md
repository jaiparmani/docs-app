...
- View traffic is extremely read-heavy and extremely skewed (a small fraction of videos get most views — same Zipf shape as [[02-url-shortener]]'s hot-URL distribution) — this is what makes CDN caching so effective here.
...
**Interview signal:** explicitly stating that these are two different systems with two different performance profiles — not "one video service" — is the structural insight everything else hangs off. This mirrors [[03-design-twitter]]'s write-path/read-path split, just for media instead of text.
...
The transcoding step is triggered the same way [[03-design-twitter]]'s fan-out is: an event (`upload_complete`) published to Kafka, consumed by a worker pool — decoupling upload completion from processing completion, so the uploader gets a fast "upload received" response without waiting for transcoding to finish.

## 6. Deep Dive: Resumable/Chunked Upload
- Large files are split into chunks (e.g. 5–10MB each) uploaded independently; the client tracks which chunks succeeded and resumes only the missing ones after a network drop — same core idea as [[Day 20 - Write-Ahead Log Implementation (LLD)|Day 20]]'s append-only, recoverable-from-a-known-point pattern, applied to a file instead of a database log.
- This is directly reusable as [[Day 42 - Object/Blob Storage Internals (HLD)|Day 42]]'s multipart upload mechanism once that day exists — object storage services (S3-style) implement exactly this: independent, resumable, parallelizable chunk uploads assembled server-side once all parts arrive.
...
- **CDN edge caching directly reuses [[Day 17 - CDN and Edge Caching (HLD)|Day 17]]** — segments for popular videos get cached at edge nodes close to viewers; unpopular/old videos fall back to origin object storage, same hot/cold split logic as any CDN use case.

## 8. Deep Dive: View Count as an Approximate, Eventually-Consistent Counter
- Incrementing a single global counter per video on every view is a write-contention bottleneck at scale, structurally the same problem [[02-url-shortener]]'s naive global-counter key generation had.
- Practical fix: each app/edge server increments a **local counter**, batched and flushed asynchronously (e.g. every few seconds) to aggregate into the real count — the displayed number is intentionally **eventually consistent and approximate**, which is acceptable because nobody makes a decision based on whether a video has exactly 1,000,004 vs 1,000,011 views. This is a direct application of [[Day 23 - CAP Theorem and PACELC (HLD)|Day 23]]'s reasoning: view count is unambiguously AP, and the interesting design move is recognizing you don't even need strong eventual consistency, just an approximately-right number refreshed periodically.
...
## Related
- [[Day 17 - CDN and Edge Caching (HLD)]] — edge caching for popular video segments
- [[Day 20 - Write-Ahead Log Implementation (LLD)]] — conceptual root of resumable/chunked writes
- [[Day 23 - CAP Theorem and PACELC (HLD)]] — approximate, AP view counters
- [[03-design-twitter]] — same upload/serving-path split pattern, same async-processing-via-queue pattern
- [[02-url-shortener]] — same global-counter contention problem reused for view counts
...
## Next
[[09-design-news-feed]] — reuses Twitter's exact fan-out skeleton, but replaces "chronological merge" with a ranking stage, and reuses this note's storage-tiering thinking for media-heavy posts.
