...
[[06-design-uber]] leaned on geohash and quadtrees without deriving them. Today builds that prerequisite properly — a genuinely new access pattern ("find things near this point") that none of Days 11–31's indexing structures (B-Tree, inverted index) are shaped for.
...
## Real-world examples
- **Redis `GEOADD`/`GEORADIUS`:** implemented as a geohash-encoded sorted set under the hood — a real, production system directly using the structure described above, and exactly what [[06-design-uber]]'s deep dive named as "the practical answer for what you'd actually run."
...
> This is the same boundary problem from [[06-design-uber]]'s quiz — restate it in your own words before expanding.
