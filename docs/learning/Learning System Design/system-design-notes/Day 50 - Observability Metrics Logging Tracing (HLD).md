...
- **ELK/OpenSearch stack (Elasticsearch, Logstash, Kibana):** the logging layer — and notice this is literally [[Day 31 - Search Systems and Elasticsearch (HLD)|Day 31]]'s inverted-index search engine, repurposed for searching structured log data instead of documents — the same "why not just grep a database" argument from that day applies here at log volume.
