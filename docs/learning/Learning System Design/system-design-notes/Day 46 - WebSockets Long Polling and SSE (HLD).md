...
Starts Block D. Every design through Day 45 was request/response: client asks, server answers, connection closes. [[07-design-chat-system]] needed something structurally different — the server pushing to a client without being asked first. Today derives that primitive properly.
...
- **The actual hard problem: statefulness.** Every prior applied design's app servers were stateless — any instance could handle any request, which is *why* [[Day 25 - Load Balancing and Reverse Proxy (HLD)|round-robin load balancing]] just works. A WebSocket connection lives on **one specific server instance** for its entire duration — that server now holds real state (the open socket) that can't just be handled by "any available instance" the way a stateless HTTP request can.
- **This statefulness is exactly what [[07-design-chat-system]]'s presence registry solves**: since a connection is pinned to one server, any other server needing to deliver a message to that user must know *which* server currently holds their socket — a routing problem that simply doesn't exist for stateless request/response designs, because there, "which server" was never a meaningful question.
