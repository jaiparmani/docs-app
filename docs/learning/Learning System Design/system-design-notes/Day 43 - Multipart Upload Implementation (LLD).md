...
Day 42 explained why object storage treats objects as immutable wholes. Today builds the client-side mechanism that reconciles that with "how do you reliably get a 2GB file into storage over a flaky connection" — the exact thing [[08-design-youtube]] referenced but didn't open up.
...
- **Resumability requires the client to persist upload progress somewhere durable of its own** (which parts succeeded, and the `upload_id`) — if the client itself crashes or the browser tab closes mid-upload, resuming requires knowing what was already confirmed, the same "durable progress tracking" requirement as [[Day 39 - Saga Orchestrator Implementation (LLD)|Day 39]]'s orchestrator persisting state after every step.
...
## Tomorrow
Day 44 (HLD) — Geospatial Indexing: a new access pattern (proximity/"near me" queries) that [[06-design-uber]] depended on, opened up properly.
