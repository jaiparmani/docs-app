...
## Explanation
- **The orchestrator must persist state after every step, not just hold it in memory.** If the orchestrator process crashes mid-Saga, it needs to resume (or at least correctly compensate) based on durable state, not lose track of what already committed — the same durability instinct as [[Day 20 - Write-Ahead Log Implementation (LLD)|Day 20]]'s WAL, applied to workflow progress instead of database rows.
