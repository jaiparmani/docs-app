...
The canonical State pattern problem — smaller and cleaner than [[02 - Design an Elevator System]], deliberately placed right after it so the same pattern is reinforced in a low-noise example before you see it combined with other complexity.
...
- **Inventory as a separate class, not folded into `VendingMachine` or the states**: this is [[01 - SOLID Principles|SRP]] — stock-tracking is a distinct responsibility from "what should happen when the user presses select," and keeping it separate means states can query it without owning it.
...
## Related
[[04 - Behavioral Design Patterns]] · [[02 - Design an Elevator System]] · [[01 - SOLID Principles]]
...
## Next
[[04 - Design Splitwise (Expense Sharing)]] — the last applied problem in this set, and the one with genuine algorithmic content (debt-graph simplification) alongside the OOD structure.
