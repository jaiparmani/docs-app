...
**Non-functional**
- Same read-heavy skew as [03-design-twitter](03-design-twitter.md) — reads vastly outnumber posts
...
- Same shape as [03-design-twitter](03-design-twitter.md)'s numbers (hundreds of millions of DAU, tens of reads per user per day) — the read:write skew argument doesn't change, so it's not worth re-deriving from scratch; the new cost center is **ranking compute per feed load**, not raw read/write throughput.
- Media storage per post is now the dominant per-item cost (a photo/video vs. 280 characters of text) — this is where [08-design-youtube](08-design-youtube.md)'s storage-tiering thinking becomes directly relevant, not just video-specific.
...
**Interview signal:** this exact two-stage shape (cheap broad retrieval, then expensive precise ranking on a small candidate set) is the same pattern named in [06-design-uber](06-design-uber.md)'s ETA-ranking follow-up — recognizing it as a *general* pattern, not a feed-specific trick, is the senior-level signal.
...
- **The celebrity/hybrid fan-out problem is unchanged** — a page with 10M followers still can't be pushed to 10M candidate stores synchronously; Twitter's hybrid push/pull split ([Section 4](03-design-twitter.md)) applies here without modification, since it's a property of fan-out, not of what happens to the candidates afterward.

## 7. Deep Dive: Media-Heavy Storage
- Photo/video posts reuse [08-design-youtube](08-design-youtube.md)'s split directly: raw upload → object storage → (for video) async transcoding → CDN-fronted serving of the media, while the **post metadata** (caption, author, timestamp, media URL) lives in a fast metadata store separate from the media blob itself.
...
## Related
- [03-design-twitter](03-design-twitter.md) — the fan-out skeleton this design reuses almost unchanged
- [08-design-youtube](08-design-youtube.md) — media storage/serving split reused for photo/video posts
- [Day 26 - Circuit Breaker Implementation (LLD)](../system-design-notes/Day 26 - Circuit Breaker Implementation (LLD).md) — graceful degradation when ranking is unavailable
- [06-design-uber](06-design-uber.md) — same two-stage filter-then-rank pattern, different domain
