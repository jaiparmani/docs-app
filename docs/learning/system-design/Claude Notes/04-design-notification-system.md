...
- Broadcast case: a single "breaking news" trigger to 50M opted-in users in a short window — this is the same **celebrity fan-out spike** from [03-design-twitter](03-design-twitter.md), just aimed outward at third-party APIs instead of an internal cache
...
**Interview signal:** recognizing that the triggering service (e.g. Order Service) should never call APNs/Twilio directly — that's a reliability leak, coupling your core business flow's availability to a third party's. The event/queue decoupling is the same pattern as [03-design-twitter](03-design-twitter.md)'s `tweet_created` → Kafka → fan-out worker, reused here for a different trigger.
...
- Each channel worker calls an **external** API you don't control — this is where [Day 26](../system-design-notes/Day 26 - Circuit Breaker Implementation (LLD).md)'s pattern applies directly: if APNs starts erroring or timing out, the circuit breaker trips, stops hammering a failing provider, and lets requests fail fast (or queue for later) instead of piling up worker threads waiting on timeouts.
- Retries use exponential backoff, capped, with the retried job carrying the **same idempotency key** as the original ([Day 24](../system-design-notes/Day 24 - Idempotency Keys (LLD).md)) — a user should never get the same push notification 4 times because the worker retried after a provider-side timeout that actually succeeded.
- Dead-letter queue for notifications that exhaust retries — not silently dropped, but held for inspection/replay, same DLQ concept from [Day 27](../system-design-notes/Day 27 - Message Queues Deep Dive (HLD).md).
...
## Related
- [03-design-twitter](03-design-twitter.md) — same fan-out/decoupling pattern, different sink
- [Day 24 - Idempotency Keys (LLD)](../system-design-notes/Day 24 - Idempotency Keys (LLD).md) — dedup on retry
- [Day 26 - Circuit Breaker Implementation (LLD)](../system-design-notes/Day 26 - Circuit Breaker Implementation (LLD).md) — isolating a flaky third-party channel
- [Day 27 - Message Queues Deep Dive (HLD)](../system-design-notes/Day 27 - Message Queues Deep Dive (HLD).md) / [Day 28 - Kafka Producer Consumer Design (LLD)](../system-design-notes/Day 28 - Kafka Producer Consumer Design (LLD).md) — the event backbone
- [Day 23 - CAP Theorem and PACELC (HLD)](../system-design-notes/Day 23 - CAP Theorem and PACELC (HLD).md) — why at-least-once + idempotency, not exactly-once, is the honest answer here
