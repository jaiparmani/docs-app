...
- **This is the resolution to [01 - SOLID Principles](01 - SOLID Principles.md)'s OCP tension**: creating a new concrete type still requires *a* code change (inside the factory), but callers are fully insulated from it — the factory is the one deliberately-accepted point of change, not scattered `if (type == ...)` checks throughout the codebase.
