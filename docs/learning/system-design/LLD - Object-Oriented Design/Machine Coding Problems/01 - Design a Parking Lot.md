...
- **[Strategy](../04 - Behavioral Design Patterns.md) for pricing**: `ParkingLot` holds a `PricingStrategy` reference, injected at construction (DIP, [01 - SOLID Principles](../01 - SOLID Principles.md)). Adding "first 30 minutes free" pricing is a new class implementing `PricingStrategy` — zero changes to `ParkingLot`.
- **[Singleton](../02 - Creational Design Patterns.md) for `ParkingLotManager`** (if this is the top-level system entry point coordinating the whole facility) — there's genuinely one physical lot, so global uniqueness is a legitimate requirement, not a lazy default.
...
## Related
[01 - SOLID Principles](../01 - SOLID Principles.md) · [02 - Creational Design Patterns](../02 - Creational Design Patterns.md) · [04 - Behavioral Design Patterns](../04 - Behavioral Design Patterns.md) · [02 - Design an Elevator System](02 - Design an Elevator System.md)
